...
function Sidebar() {
  ...
  const adminMenuItems = [
    ...
    { icon: BedDouble, label: "Beds", path: "/beds" },
    { icon: Pill, label: "Pharmacy", path: "/pharmacy" },
    { icon: BarChart3, label: "Reports", path: "/analytics" },
    { icon: MessageSquare, label: "Messages", path: "/admin?tab=messages" },
  ...
  {user?.profile_image ? (
    ...
    `${user?.firstName?.[0] || ""}${user?.lastName?.[0] || ""}`
          ...
          <div className="min-w-0">
            <p className="text-sm text-slate-700 truncate" style={{ fontWeight: 600 }}>
              {user?.role === "admin"
                ? "Administrator"
                : user?.role === "doctor"
